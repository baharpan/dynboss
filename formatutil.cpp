...
void getKmers(vector< string >& reads, unsigned K, unordered_set<kmer_t>& kmers) {
   cout << "Erasing all 'N' characters..." << endl;
   kmers.clear();
   for (auto r : reads) {
...
{
   cout << "Writing binary k-mers to file: " << outfile << endl;
  ...
  cout << "Verifying file " << outfile << "..." << endl;
  ...
  cout << "File has been successfully verified." << endl;
}

void read_from_bin(string filename, unordered_set<kmer_t> &out)
{
   cout << "Reading mers from binary file " << filename << " ..." << endl;
  ...
  cout << "File size (Gb): " << nbytes / (1024.0*1024.0*1024.0) << endl;
  ...
  cout << "Input finished, " << kmer_vec.size() << " mers read." << endl;
}
...
void get_kmers_fasta_or_bin(string fasta_file, unsigned k, unordered_set<kmer_t> &out)
{
  ...
  {
     cout << "Binary file for " << k << "-mers does not exist ..." << endl;
    // get reads from file
