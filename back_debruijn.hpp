...
#ifndef DYNAMIC //different template defaults for dynamic dBG
template <size_t t_sigma            = 4, // default: DNA, TODO: change to 0 and make dynamic
...
#else
   template <size_t t_sigma            = 4, // default: DNA, TODO: change to 0 and make dynamic
	     class  t_bit_vector_type  = gap_bv,
          class  t_bv_rank_type     = gap_bv, // We invert the bits so it is 0 instead
          class  t_bv_select_type   = gap_bv,
	     class  t_edge_vector_type = wt_str,
          class  t_symbol_type      = typename t_edge_vector_type::value_type,
          class  t_label_type       = string> // can define basic_string<t_symbol_type>, but need to use correct print func
#endif
...
#else
      t_bit_vector_type* p_node_flags;
      t_edge_vector_type* p_edges;
      
      //To provide same interface as static dbg
      t_bit_vector_type& m_node_flags; //= *p_node_flags
      t_edge_vector_type& m_edges;     //= *p_edges

      array<size_t, 1+sigma> m_symbol_ends{};
      //size_t m_symbol_ends[1 + sigma ];
  ...
  private:
#ifndef DYNAMIC
      debruijn_graph(size_t in_k,
		     const t_bit_vector_type & node_flags,
		     t_edge_vector_type & edges,
		     const array<size_t, 1+sigma>& symbol_ends,
		     const label_type& alphabet)
	 : k(in_k),
	   m_node_flags(node_flags),
	   m_node_rank(&m_node_flags),
	   m_node_select(&m_node_flags),
	   m_edges(edges),
	 ...
	 m_edge_max_ranks(_init_max_ranks(*edges)),
	 m_alphabet( alphabet ),
	 m_num_nodes(m_node_flags.rank0())
      {}
#endif

  array<size_t, 1+sigma> _init_max_ranks(t_edge_vector_type & edges) {
...
#else
    t_bit_vector_type* bv = new t_bit_vector_type(first);
    t_edge_vector_type* wt = new t_edge_vector_type(16, edges ); //16 is the alphabet size
#endif
  ...
  label_type node_label_from_edge(size_t i) {
    label_type label = label_type(k-1, _map_symbol(symbol_type{}));
    return _node_label_from_edge_given_buffer(i, label);
  }
  ...
  label_type _node_label_from_edge_given_buffer(size_t i, label_type & label) {
...
#else
    return m_node_flags.rank0(b) - m_node_flags.rank0(a);
#endif
  }
...
#else
    size_t next  = m_node_flags.select0(m_node_flags.rank0(start+1) + nth);
#endif
    return next;
  }
  ...
  size_type size() const { return num_edges(); }
   ...
   bool index(InputIterator in) const {
	 ...
	 size_t dSize = 0;
#ifndef DYNAMIC
	 //use sdsl size functions
	 dSize += size_in_bytes( m_node_flags ) * 8;
	 dSize += size_in_bytes( m_edges ) * 8;
#else
	 
	 dSize += m_node_flags.bit_size();
	 dSize += m_edges.bit_size();
